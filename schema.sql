...
CREATE TABLE fundraisers (
  ...
  password_hash TEXT NOT NULL,
  phone         VARCHAR(50) UNIQUE,
  role          VARCHAR(30) DEFAULT 'fundraiser', -- có thể: 'fundraiser','hospital_staff'
  created_at    TIMESTAMP NOT NULL DEFAULT NOW()
);
...
CREATE TABLE raise_tickets (
  ...
  patient_id      BIGINT NOT NULL REFERENCES patients(patient_id),
  fundraiser_id   BIGINT NOT NULL REFERENCES fundraisers(fundraiser_id),
  hospital_id     BIGINT NOT NULL REFERENCES hospitals(hospital_id),
  ...
  created_by_id   BIGINT REFERENCES fundraisers(fundraiser_id),   -- người tạo (có thể = fundraiser_id)
...
CREATE TABLE ticket_updates (
  ...
  created_by      BIGINT REFERENCES fundraisers(fundraiser_id),
  created_at      TIMESTAMP NOT NULL DEFAULT NOW()
);
